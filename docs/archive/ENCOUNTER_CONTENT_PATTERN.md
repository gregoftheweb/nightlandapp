...
interface EncounterShapeAdapter<TRawEntry, TShapeConfig> {
  ...
  // Explicit route contract. Does NOT assume routes can be derived by
...
**The catalog holds untrusted, unparsed content until validated.** Static TypeScript registration does not imply validity — imported content is still authored data that must go through `adapter.parse()`.
