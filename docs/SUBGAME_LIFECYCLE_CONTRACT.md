...
```ts
type SubGameShapeId = 'dialogue' | 'word-grid' | 'timed-encounter' | 'one-off'
...
`jaunt-crystal-grant` dispatches `GRANT_JAUNT_CRYSTAL`. Its durable per-instance reward flag keeps
each encounter idempotent while allowing distinct timed-encounter instances to grant additional
reserve crystals.
...
| Jaunt Cave    | `jaunt-cave`; `timed-encounter`; `/sub-games/jaunt-cave/jaunt-cave`          | Player confirms return from victory screen; idempotent            | Death; killer `Jaunt Daemon`; declared dialog suppression and route            | No                                                                                                | `aftermath-screen`                                                                          | `local-only`; battle restarts after remount before completion               | `jaunt-crystal-grant`; idempotent per instance completion              | Victory/aftermath exits use resume signal and `exitSubGame`; death uses death flow                   |
