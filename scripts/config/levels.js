...
exports.levels = exports.createItemInstance = void 0;
exports.getLevel = getLevel;
var objects_1 = require("./objects");
var monsters_1 = require("./monsters");
var levelHelpers_1 = require("./levelHelpers");
// Helper function to create object instances from building templates
var createObjectInstance = function (templateShortName, position, overrides) {
    if (overrides === void 0) { overrides = {}; }
    var template = (0, objects_1.getBuildingTemplate)(templateShortName);
    if (!template) {
        throw new Error("Building template ".concat(templateShortName, " not found"));
    }
    return __assign({ id: "".concat(template.shortName, "_").concat(position.row, "_").concat(position.col), templateId: templateShortName, position: position, active: true, shortName: template.shortName, category: template.category, name: template.name, description: template.description, image: template.image, size: template.size || {
            width: template.width || 1,
            height: template.height || 1,
        }, zIndex: template.zIndex, effects: template.effects, subGame: template.subGame }, overrides);
};
// Helper function to create item instances from templates
var createItemInstance = function (templateShortName, position, overrides) {
    if (overrides === void 0) { overrides = {}; }
    // Try weapon first, then consumable
    var template = (0, objects_1.getWeaponTemplate)(templateShortName);
    var itemType = 'weapon';
    if (!template) {
        template = (0, objects_1.getConsumableTemplate)(templateShortName);
        itemType = 'consumable';
    }
    if (!template) {
        template = (0, objects_1.getCollectibleTemplate)(templateShortName);
        itemType = 'collectible';
    }
    if (!template) {
        throw new Error("Item template ".concat(templateShortName, " not found"));
    }
    var baseItem = {
        ...
        type: itemType,
        collectible: true,
        id: "".concat(template.shortName, "_").concat(position.row, "_").concat(position.col),
    ...
    if (itemType === 'weapon') {
        ...
        var healEffect = template.effects.find(function (e) { return e.type === 'heal'; });
        if (healEffect) {
            baseItem.healAmount = healEffect.value;
        }
    }
    console.log('adding images:', baseItem.image);
    return __assign(__assign({}, baseItem), overrides);
};
exports.createItemInstance = createItemInstance;
// Helper function to create monster spawn configs - WITH SPAWN SETTINGS
var createMonsterInstance = function (monsterShortName, spawnRate, maxInstances, position) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j;
    if (position === void 0) { position = { row: 0, col: 0 }; }
    var template = (0, monsters_1.getMonsterTemplate)(monsterShortName);
    if (!template) {
        throw new Error("Monster template ".concat(monsterShortName, " not found"));
    }
    return {
        id: "".concat(monsterShortName, "_spawn_config"),
        templateId: monsterShortName,
        position: position,
        active: true,
        currentHP: (_a = template.hp) !== null && _a !== void 0 ? _a : 10,
        hp: (_b = template.hp) !== null && _b !== void 0 ? _b : 10,
        maxHP: (_d = (_c = template.maxHP) !== null && _c !== void 0 ? _c : template.hp) !== null && _d !== void 0 ? _d : 10,
        shortName: template.shortName,
        category: template.category,
        name: template.name,
        description: template.description,
        image: template.image,
        attack: (_e = template.attack) !== null && _e !== void 0 ? _e : 0,
        ac: (_f = template.ac) !== null && _f !== void 0 ? _f : 0,
        moveRate: (_g = template.moveRate) !== null && _g !== void 0 ? _g : 1,
        soulKey: (_h = template.soulKey) !== null && _h !== void 0 ? _h : '000000',
        initiative: (_j = template.initiative) !== null && _j !== void 0 ? _j : 0,
        // SPAWN CONFIGURATION - SET PER LEVEL
        spawnRate: spawnRate, // Percentage chance (0.0 to 1.0) that monster spawns each turn
        maxInstances: maxInstances,
        spawned: false,
    };
};
var createNonCollisionObject = function (templateName, position, rotation, overrides) {
    if (overrides === void 0) { overrides = {}; }
    var template = (0, objects_1.getNonCollisionTemplate)(templateName);
    if (!template) {
        throw new Error("NonCollisionObject template ".concat(templateName, " not found"));
    }
    return __assign(__assign({ id: "".concat(template.shortName, "_").concat(position.row, "_").concat(position.col, "_").concat(rotation), position: position, rotation: rotation }, template), overrides);
};
// Helper function to create GreatPower instances for levels
var createGreatPowerForLevel = function (shortName, position, overrides) {
    if (overrides === void 0) { overrides = {}; }
    var template = (0, monsters_1.getGreatPowerTemplate)(shortName);
    if (!template) {
        throw new Error("GreatPower template ".concat(shortName, " not found"));
    }
    return __assign(__assign(__assign({}, template), { id: "".concat(shortName, "_").concat(position.row, "_").concat(position.col), position: position }), overrides);
};
/**
...
exports.levels = {
    '1': {
        ...
        description: 'The only lands known by the Monstruwacans, all else is beyond they skill and ken.',
        ...
        items: [
            (0, exports.createItemInstance)('healthPotion', { row: 395, col: 195 }),
            (0, exports.createItemInstance)('ironSword', { row: 380, col: 200 }),
            (0, exports.createItemInstance)('maguffinRock', { row: 390, col: 210 }),
        ],
        // MONSTERS - Use spawn table for normalized configurations
        monsters: (0, levelHelpers_1.loadSpawnTable)('wasteland_common', createMonsterInstance),
    ...
    '2': {
        ...
        items: [],
        // MONSTERS - Use spawn table for normalized configurations
        monsters: (0, levelHelpers_1.loadSpawnTable)('grounds_common', createMonsterInstance),
...
[exports.levels['1'], exports.levels['2']].forEach(levelHelpers_1.validateLevel);
/**
