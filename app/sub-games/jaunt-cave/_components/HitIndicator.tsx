...
export const HitIndicator: React.FC<HitIndicatorProps> = ({ position, type, onComplete }) => {
      ...
      {type === 'block' ? (
        ...
        <>
          {Array.from({ length: HIT_INDICATOR_CONFIG.HIT_RING_COUNT }).map((_, i) => {
            const radius = halfSize - i * HIT_INDICATOR_CONFIG.HIT_RING_SPACING
          ...
          <Text style={[styles.hitText, { color: HIT_INDICATOR_CONFIG.HIT_TEXT_COLOR }]}>HIT</Text>
        </>
      )}
    </Animated.View>
  )
