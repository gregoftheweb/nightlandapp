...
const JauntCaveScreen2: React.FC<JauntCaveScreen2Props> = ({
  ...
  const getHitIndicatorYNudge = (w: number, h: number) => {
    const aspect = h / Math.max(1, w)

    // Square-ish: ~10px, Tall: ~30px
    const aspectMin = 1.0
    const aspectMax = 2.0
  ...
  const adjustedHitPos =
    hitIndicator?.position && arenaSize
      ? {
          x: hitIndicator.position.x,
          y: hitIndicator.position.y + getHitIndicatorYNudge(arenaSize.width, arenaSize.height),
        }
      : (hitIndicator?.position ?? null)
  ...
  return (
    <BackgroundImage source={BACKGROUND} overlayOpacity={0}>
      <View style={styles.container} onLayout={handleArenaLayout}>
        ...
        {/* Hit Indicator */}
        <HitIndicator position={adjustedHitPos} type={hitIndicator?.type ?? 'block'} />
