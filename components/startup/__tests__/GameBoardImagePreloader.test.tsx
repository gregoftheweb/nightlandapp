...
describe('GameBoardImagePreloader', () => {
  ...
  test('covers 26 distinct gameboard image files and waits for every onLoad', () => {
    const onReady = jest.fn()
    const screen = render(<GameBoardImagePreloader onReady={onReady} />)

    expect(GAMEBOARD_IMAGE_ASSETS).toHaveLength(26)
    expect(new Set(GAMEBOARD_IMAGE_ASSETS.map(({ id }) => id)).size).toBe(26)
    ...
    expect(onReady).toHaveBeenCalledWith({ reason: 'loaded', loadedCount: 26, failedIds: [] })
  })
